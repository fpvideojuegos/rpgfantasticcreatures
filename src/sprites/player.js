    ...
    if (this.alive) {
      ...
      if (this.canLoad) 
      {
        if (this.x > this.scene.physics.world.bounds.width) {
          this.scene.registry.set('load', this.scene.map.properties[MapConstants.loadRight].value);
          this.scene.registry.set('spawn', this.scene.map.properties[MapConstants.spawnRight].value);
          this.canLoad = false;
          this.scene.end('restart');
        } else if (this.x < 0) {
          this.scene.registry.set('load', this.scene.map.properties[MapConstants.loadLeft].value);
          this.scene.registry.set('spawn', this.scene.map.properties[MapConstants.spawnLeft].value);
          this.canLoad = false;
          this.scene.end('restart');
        } else if (this.y > this.scene.physics.world.bounds.height) {
          this.scene.registry.set('load', this.scene.map.properties[MapConstants.loadDown].value);
          this.scene.registry.set('spawn', this.scene.map.properties[MapConstants.spawnDown].value);          
          this.canLoad = false;
          this.scene.end('restart');
        } else if (this.y < 0) {
          this.scene.registry.set('load', this.scene.map.properties[MapConstants.loadUp].value);
          this.scene.registry.set('spawn', this.scene.map.properties[MapConstants.spawnUp].value);
          this.canLoad = false;
          this.scene.end('restart');
        }
      }
